	...
	<div class="container">
		...
		<form method="post" action="list.do">
			<div class="search">
				<select name="col">
					...
					<option value="skills"
						<c:if test="${col=='skills' }">selected</c:if>>보유기술</option>
		...
		<table class="table table-bordered">
			...
			<c:forEach var="dto" items="${list }">
				...
				<tr>
					<td>${dto.no}</td>
					<td style = "width:30%;"><c:forEach begin="1" end="${dto.indent }">&nbsp;&nbsp;
					</c:forEach> <c:if test="${dto.indent>0 }">
							<img src='../images/re.jpg'>
						</c:if> <a href="javascript:read('${dto.no}')"> ${dto.name}</a>
					<td>${dto.gender}</td>
					<td>${dto.phone}</td>
					<td>${dto.skills}</td>
					<td><a href="javascript:update('${ dto.no}')">수정</a> / <a
						href="javascript:del('${ dto.no}')">삭제</a> / <a
						href="javascript:reply('${ dto.no}')">답변</a></td>
				</tr>
			</c:forEach>
		</table>
		<div>${paging }</div>
